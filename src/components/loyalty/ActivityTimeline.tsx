...
export default function ActivityTimeline({
  ...
  const getActivityIcon = (type: LoyaltyActivity['type']) => {
    switch (type) {
      case 'earned':
        return (
          <div className="p-2 rounded-full bg-cookies-yellow/20">
            <svg className="w-4 h-4 text-cookies-yellow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {displayActivities.length === 0 ? (
        ...
        <>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {activity.orderId && (
                    <LocalizedClientLink
                      ...
                      className="text-cookies-yellow text-xs hover:text-cookies-light-yellow"
                    >
                      View Order →
                    </LocalizedClientLink>
                  )}
          ...
          {showViewAll && activities.length > limit && (
            <LocalizedClientLink
              href="/user/rewards"
              className="block text-center py-3 text-cookies-yellow hover:text-cookies-light-yellow font-bold text-sm"
            >
